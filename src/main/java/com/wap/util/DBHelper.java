...
public class DBHelper {
    ...
    private Connection getConnection(){
        ...
        try {
            InputStream fis=this.getClass().getResourceAsStream("/jdbc.properties"); //加载数据库配置文件到内存中
            Properties p=new Properties();
            p.load(fis);

            driver_class=p.getProperty("driverClasss");   //获取数据库配置文件
            driver_url=p.getProperty("jdbcUrl");
            database_user=p.getProperty("username");
